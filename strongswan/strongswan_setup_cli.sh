...
function rewrite_config {
    ...
    set vpn ipsec esp-group ${ESP_GROUP_NAME} lifetime 3600
    set vpn ipsec esp-group ${ESP_GROUP_NAME} compression disable


    # setup IKE group
    set vpn ipsec ike-group ${IKE_GROUP_NAME} proposal 1 dh-group 21
    set vpn ipsec ike-group ${IKE_GROUP_NAME} proposal 1 encryption aes256 
    set vpn ipsec ike-group ${IKE_GROUP_NAME} proposal 1 hash sha384
    set vpn ipsec ike-group ${IKE_GROUP_NAME} key-exchange ikev2
    set vpn ipsec ike-group ${IKE_GROUP_NAME} lifetime 28800
    set vpn ipsec ike-group ${IKE_GROUP_NAME} dead-peer-detection action clear
    set vpn ipsec ike-group ${IKE_GROUP_NAME} dead-peer-detection interval 15
    set vpn ipsec ike-group ${IKE_GROUP_NAME} dead-peer-detection timeout 30
